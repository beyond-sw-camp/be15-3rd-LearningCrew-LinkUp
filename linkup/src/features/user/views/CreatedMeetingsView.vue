...
import { getMyCreatedMeetings } from '@/api/user';
...
const fetchCreatedMeetings = async (page, size) => {
  const status = tabStatusMap[activeTab.value];
  return await getMyCreatedMeetings({ status, page, size });
};
...
const {
  ...
  hasNext,
  isLoading,
} = usePagination(fetchCreatedMeetings);
...
const meetingStatusClass = (status) => {
  switch (status) {
    case 'PENDING':
      return 'upcoming';
    case 'DONE':
...
<template>
  <section class="history-container" aria-label="개설 모임 이력 조회 섹션">
    <!-- 헤더 -->
    <header class="history-header">
      <h2 class="history-title">내가 개설한 모임</h2>
